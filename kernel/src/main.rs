...
use logger::EmbeddedCollector;
use tracing::{error, info, trace, warn, Dispatch};
use virtio_drivers::{
	device::{blk::VirtIOBlk, gpu::VirtIOGpu},
	transport::{
		pci::{
			bus::{
				BarInfo, Cam, Command, ConfigurationAccess, DeviceFunction,
				MmioCam, PciRoot,
			},
...
extern "C" fn main(_mbi: *const u8) -> ! {
	...
	let subscriber = EmbeddedCollector::new();
	let dispatch = Dispatch::new(subscriber);
	tracing::dispatch::set_global_default(dispatch)
...
fn virtio_blk<T: Transport>(transport: T) {
	let mut blk = VirtIOBlk::<HalImpl, T>::new(transport)
...
fn virtio_gpu<T: Transport>(transport: T) {
	let mut gpu = VirtIOGpu::<HalImpl, T>::new(transport)
...
fn virtio_net<T: Transport>(transport: T) {
	#[cfg(not(feature = "tcp"))]
	{
		let mut net = virtio_drivers::device::net::VirtIONetRaw::<
			HalImpl,
			T,
			NET_QUEUE_SIZE,
		>::new(transport)
		...
		let (hdr_len, pkt_len) =
			net.receive_wait(&mut buf).expect("failed to recv");
		info!(
			"recv {} bytes: {:02x?}",
			pkt_len,
	...
	#[cfg(feature = "tcp")]
	{
		...
		let net = virtio_drivers::device::net::VirtIONet::<
			HalImpl,
			T,
			NET_QUEUE_SIZE,
...
fn enumerate_pci(mmconfig_base: *mut u8) {
	...
	let mut pci_root =
		PciRoot::new(unsafe { MmioCam::new(mmconfig_base, Cam::Ecam) });
	for (device_function, info) in pci_root.enumerate_bus(0) {
		...
		if let Some(virtio_type) = virtio_device_type(&info) {
			...
			let mut transport =
				PciTransport::new::<HalImpl, _>(&mut pci_root, device_function)
					.unwrap();
			info!(
